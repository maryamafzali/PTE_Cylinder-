function [E,J]=slow_exchange_b_tensor_SynthMeasWatsonHinderedDiffusion_PGSE(x, grad_dirs, b, fibredir,b_delta, mask_b_delta)
...
% Parallel component
if nargout > 1
    [LePar, J_LePar] = b_tensor_CylNeumanLePar_PGSE(De_par - De_per, b, b_delta);
else
    LePar = b_tensor_CylNeumanLePar_PGSE(De_par - De_per, b, b_delta);
end
...
% added by Maryam
E = 0.5*E.*ePerp.* exp(b/3.*b_delta.*(De_par - De_per)-b/3.*(De_par+2*De_per));
