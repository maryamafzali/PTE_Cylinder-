function [E,J]=b_tensor_R_SynthMeasWatsonSHCylNeuman_PGSE(x, grad_dirs, G, delta, smalldel, fibredir, roots, b_delta)
...
% BesselJ_RootsCyl.
%
% Main code is written by Gary Hui Zhang (gary.zhang@ucl.ac.uk)
% The code edited by Maryam Afzali (maryam.afzali.m@gmail.com) to consider
% b_delta
...
Gamma = 2.675987E8;
td=(delta-(smalldel/3));        %(in seconds)
b=(td).*((Gamma.*smalldel.*G).^2); %(sec/mircometer^2)
...
% if nargout > 1
%     [LePar, J_LePar] = b_tensor_CylNeumanLePar_PGSE(d, b,b_delta);
% else
%     LePar = b_tensor_CylNeumanLePar_PGSE(d, b,b_delta);
% end

% Perpendicular component
% LePerp = zeros(size(b,1),1);
% J_LePerp = zeros([size(LePerp), 2]);
% J_LePerp(:,:,:) = 0;
...
coeff(coeff == 0) = 3;
coeff(coeff == -0.5) = 2;
ePerp = exp(LePerp/3*(b_delta+2).*coeff);

% Compute the Legendre weighted signal
Lpmp = b_delta*(LePerp - LePar);

lgi = cmplx_LegendreGaussianIntegral(Lpmp, 6);
...
E = 0.5*E.*ePerp.* exp(-b/3.*d.*(1-b_delta));
